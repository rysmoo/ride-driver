...
import com.rideke.driver.common.helper.CircularMusicProgressBar
import com.rideke.driver.common.helper.CustomDialog
import com.rideke.driver.common.helper.WaveDrawable
...
import com.rideke.driver.trips.timelineview.LocationTimeLineView
...
class RequestReceiveActivity : CommonActivity(), LinearTimer.TimerListener, ServiceListener, OnMapReadyCallback {
    ...
    lateinit @BindView(R.id.etd_distance)
    var etd_distance: TextView

    lateinit @BindView(R.id.tvPaymentMethod)
    var tvPaymentMethod: TextView

    lateinit @BindView(R.id.etd_fare)
    var etd_fare: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        etd_distance = findViewById<View>(R.id.etd_distance) as TextView
        etd_fare = findViewById<View>(R.id.etd_fare) as TextView
        tvPaymentMethod = findViewById<View>(R.id.tvPaymentMethod) as TextView
        ...
        try {
            ...
            if (jsonObject.getJSONObject("custom").has("ride_request")) {
                etdFare = jsonObject.getJSONObject("custom").getJSONObject("ride_request").getString("fare_estimation")
                etdDistance = jsonObject.getJSONObject("custom").getJSONObject("ride_request").getString("distance")
                ...
                if (!paymentMode.equals("")) tvPaymentMethod.text = getString(R.string.payment_methods) + ": " + paymentMode
                else tvPaymentMethod.visibility=View.GONE
                ...
                if (etdFare!=""&&etdFare!="0"&&etdFare!="0.0") {
                    etd_fare.text =  resources.getString(R.string.estimated_fare)+ ": " +sessionManager.currencySymbol+etdFare
                } else {
                    etd_fare.visibility=View.GONE
                }

                if (etdDistance != "" && etdDistance != "0" && etdDistance != "0.0") {
                    try {
                        etd_distance.text = String.format("%.2f", (etdDistance.toFloat()/1000)) + " km"
                    }catch (e:Exception){
                        etd_distance.text = "$etdDistance meters"
                    }
                } else {
                    etd_distance.visibility = View.GONE
                }
...
//        locationHashMap["pickup_longitude"] = pickup?.longitude.toString()
    ...
    fun accept() {
        ...
        drawable.setColorFilter(ContextCompat.getColor(this, R.color.cabme_app_yellow), PorterDuff.Mode.SRC_IN)
