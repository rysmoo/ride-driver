...
import com.rideke.driver.common.custompalette.FontCache
   ...
   */
    override fun onBindViewHolder(viewHolder: PriceRecycleAdapter.ViewHolder, i: Int) {
        ...
        if (feedbackarraylist[i].colour == "black") {
            ...
            viewHolder.fareAmt.setTextColor(context.resources.getColor(R.color.cabme_app_black_dark))
            viewHolder.faretxt.setTextColor(context.resources.getColor(R.color.cabme_app_black_dark))
            viewHolder.fareAmt.setTextSize(16F)
            viewHolder.faretxt.setTextSize(16F)
        }
        if (feedbackarraylist[i].colour == "green" || feedbackarraylist[i].colour == "yellow") {
            viewHolder.fareAmt.typeface = FontCache.getTypeface(context.resources.getString(R.string.fonts_UBERMedium), context)
            viewHolder.faretxt.typeface = FontCache.getTypeface(context.resources.getString(R.string.fonts_UBERMedium), context)
            viewHolder.fareAmt.setTextColor(context.resources.getColor(R.color.cabme_app_yellow))
            viewHolder.faretxt.setTextColor(context.resources.getColor(R.color.cabme_app_yellow))
            viewHolder.fareAmt.setTextSize(16F)
            viewHolder.faretxt.setTextSize(16F)
        }
