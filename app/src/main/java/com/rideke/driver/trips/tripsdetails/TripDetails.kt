...
import com.rideke.driver.common.util.CommonMethods.Companion.showInternetNotAvailableForStoredDataViewer
import com.rideke.driver.common.util.CommonMethods.Companion.showNoInternetAlert
...
import com.rideke.driver.trips.rating.PriceRecycleAdapter
 ...
 */
    ...
    private fun onSuccessTripDetail(jsonResponse: String) {
        ...
        if (!(tripDetailsModels?.riderDetails?.get(0)?.mapImage?.contains(".png"))!! ||TextUtils.isEmpty(tripDetailsModels?.riderDetails?.get(0)?.mapImage)) {
    ...
    private fun checkAddress(
        ...
        try {
            if (tripDetailsModels?.riderDetails?.get(0)?.pickupAddress.equals("") || tripDetailsModels?.riderDetails?.get(0)?.destAddress.equals(""))
            {
                pikcuplatlng?.let {
